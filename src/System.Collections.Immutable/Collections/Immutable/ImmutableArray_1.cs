...
namespace System.Collections.Immutable
{
    public partial struct ImmutableArray<T> : IReadOnlyList<T>, IList<T>, IEquatable<ImmutableArray<T>>, IList, IImmutableArray, IStructuralComparable, IStructuralEquatable, IImmutableList<T>
    {
        ...
        public ImmutableArray<T> Sort(Comparison<T> comparison)
        {
            ...
            return self.Sort(new ComparisonComparer<T>(comparison));
        }
        ...
        static int CompareTo(Array thisArray, object other, IComparer comparer)
        {
            ...
            Array arrays = other as Array;
            if (arrays == null || thisArray.Length != arrays.Length)
            {
                throw new ArgumentException("other");
            }
            int num = 0;
            int num1 = 0;
            while (num < arrays.Length && num1 == 0)
            {
                object value = thisArray.GetValue(num);
                num1 = comparer.Compare(value, arrays.GetValue(num));
                num++;
            }
            return num1;
        }

        static bool Equals(Array thisArray, object other, IEqualityComparer comparer)
        {
            if (other == null)
            {
                return false;
            ...
            Array arrays = other as Array;
            if (arrays == null || arrays.Length != thisArray.Length)
            {
                return false;
            }
            for (int i = 0; i < arrays.Length; i++)
            {
                if (!comparer.Equals(thisArray.GetValue(i), arrays.GetValue(i)))
                {
                    return false;
                }
            }
        ...
        static int GetHashCode(Array thisArray, IEqualityComparer comparer)
        {
            if (comparer == null)
            {
            ...
            int num = 0;
            for (int i = (thisArray.Length >= 8 ? thisArray.Length - 8 : 0); i < thisArray.Length; i++)
            {
                num = CombineHashCodes(num, comparer.GetHashCode(thisArray.GetValue(i)));
            }
            return num;
        }
